...
def supervised_model_training(x_train, y_train, x_test, 
  ...
  if problem_type == "Classification":
    if len(np.unique(y_train))>2:
      ...
      # print("length and shape: ", len(np.unique(y_test)), len(np.unique(y_train)), predict.shape)
      # print(np.unique(y_test), np.unique(y_train))
      # auc = metrics.roc_auc_score(y_test, predict[:,0:len(np.unique(y_test))],average="weighted",multi_class="ovr")
...
def stat_sim(real_path,fake_path,cat_cols=None):
    ...
    for column in real.columns:
        ...
        if column in cat_cols:
            ...
            fake_data_categories =  (fakey[column].value_counts()/fakey[column].value_counts().sum()).keys().tolist()
            ...
            for i in sorted_categories:
                ...
                if i not in fake_data_categories:
    ...
    return [np.nanmean(num_stat),np.nanmean(cat_stat),corr_dist]
