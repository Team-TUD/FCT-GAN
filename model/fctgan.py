"""
Generative model training algorithm based on the FCTGANSynthesizer
...
class FCTGAN():
    ...
    def __init__(self,
        ...
        self.synthesizer = FCTGANSynthesizer()
    ...
    def fit(self):
        ...
        general = self.data_prep.column_types["general"], non_categorical = self.data_prep.column_types["non_categorical"], type=self.problem_type, data_prep = self.data_prep)
        end_time = time.time()
    ...
    def generate_samples_nolimit(self):
        
        sample = self.synthesizer.sample_fast(len(self.raw_df)) 
        sample_df = self.data_prep.inverse_prep(sample)
