...
class DataTransformer():
    ...
    def fit(self):
        ...
        for id_, info in enumerate(self.meta):
            if info['type'] == "continuous":
                '''
                no_g and yes_g mean if it is general transform
                '''
                ...
                self.total_categories += info['size']
    ...
    def inverse_transform_fast(self, data):
        data_t = np.zeros([len(data), len(self.meta)])
        invalid_ids = []
        st = 0
        for id_, info in enumerate(self.meta):
        ...
        invalid_ids = [] 
        # np.unique(np.array(invalid_ids)) 
        all_ids = np.arange(0,len(data))
        valid_ids = list(set(all_ids) - set(invalid_ids))
...
class ImageTransformer():
    ...
    def __init__(self, side, fno: int):
    ...
    def transform(self, data):
        ...
        if self.fno == 1:
            ...
            return data.view(-1, self.height, self.height, 1) if self.fno == 2 else data.view(-1, 1, self.height, self.height)
