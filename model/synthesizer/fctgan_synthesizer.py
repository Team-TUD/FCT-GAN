...
Conv2d, ConvTranspose2d, Sigmoid, init, BCELoss, CrossEntropyLoss,SmoothL1Loss, BatchNorm2d, GELU, LayerNorm, Identity, ModuleList, Parameter)
...
class CFNO2d(Module):
    def __init__(self, side, dim, drop_path=0.2, dropout=0.2, mlp_ratio = 4.0, depth = 4, embed_dim = 256):
        ...
        # We know for exmaple Adult dataset is wrapped as 24*24, with patch_size = 8, there will have 9 (3*3) patches after.
        # The image size, patch size will influence the setting in GFGenerator() function. Unfortunately, user may need to manually find the parameters for their own datasets.
        self.patch_size = 8
        ...
        self.blocks = ModuleList([Block(self.img_size//self.patch_size, dim, drop_path, dropout, mlp_ratio) for i in range(depth)])
...
class BlockGenerator(Module):
    ...
    def __init__(self, side, dim = 64, drop_path = 0.2, dropout=0.2, mlp_ratio = 4.0, output_features = -1):
        ...
        self.drop_path = DropPath(drop_path) if drop_path > 0 else Identity()
        self.norm2 = LayerNorm(dim)
        self.mlp = Mlp(in_features=dim, hidden_features=int(dim*mlp_ratio), drop=dropout) if output_features == -1 else Mlp(in_features=dim, hidden_features=int(dim*mlp_ratio), out_features = output_features, drop=dropout)
...
class GFGenerator(Module):
    ...
    def forward(self, z):
        ...
        output = self.deconv(x.view(-1, self.bottom_width*4, self.bottom_width*4, self.embed_dim//16).permute(0,3,1,2))
...
def determine_layers_disc(side, num_channels, fno: int):
    ...
    if fno == 1:
        ...
        layers_D += [CFNO2d(side, num_channels, drop_path=0.2, dropout=0.2, mlp_ratio=4, depth=4)] # no sigmoid here since we wgan+gp as loss for discriminator
    else:
        num_channels = 64
...
class FCTGANSynthesizer:
    def __init__(self,
                 ...
                 batch_size=500,
                 epochs=150,
                 gen_fno=2, # Here we set gen_fno and disc_fno to 2 to use 2D FNO. If they are set to 0, they use the same generator and discriminator structures as CTAB-GAN+
                 disc_fno=2):
    ...
    def fit(self, train_data=pd.DataFrame, categorical=[], mixed={}, general=[], non_categorical=[], type={}, data_prep=None):
        ...
        layers_G = determine_layers_gen(self.gside, self.random_dim+self.cond_generator.n_opt, self.num_channels, fno=self.gen_fno)
        ...
        self.Gtransformer = ImageTransformer(self.gside, self.gen_fno)       
        self.Dtransformer = ImageTransformer(self.dside, self.disc_fno)
